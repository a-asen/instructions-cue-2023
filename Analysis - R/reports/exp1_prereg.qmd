...
A total of
`r loss$exclude_par_trials + loss$rt_sd_trials + loss$inducer_fail_trials` (ignoring excluded participants, we are left with: `r loss$rt_sd_trials + loss$inducer_fail_trials`)
trials were lost. Representing a loss of
`r round(loss$exclude_par_pct + loss$rt_sd_pct + loss$inducer_fail_pct, 2)` (or `r round(loss$rt_sd_pct + loss$inducer_fail_pct, 2)`)
percent of the data.
...
d |>
  ...
  scale_x_continuous(breaks = c(0,1), labels = c("Incongruent", "Congruent"), name="",
                     expand = c(.2,.2), minor_breaks = NULL)+
  scale_y_continuous(breaks = seq(0,1,.01), minor_breaks = NULL)+
