...
We recruited `r length(dem$age)` participants (`r age$female` females) with a mean age of `r dem$age` (SD = `r dem$age_sd`) from the United Kingdom via Prolific. 
...
d |>
  ...
  mutate(correct_response = ifelse(is.na(correct_response), 0, correct_response)) |>
    # non-response are counted as missing 
...
```
*Note.* The tests have `r  d2_df` degrees of freedom.  \n
...
d |>
  ...
  stat_summary(aes(col=NULL), position=position_dodge(.1), geom = "point", fun = mean, alpha = .2)+
  stat_summary(aes(col=NULL), position=position_dodge(.1), geom = "line", fun = mean, alpha = .2)+
  stat_summary(aes(group=NULL), fun.data = mean_se, geom = "errorbar", width=.2, size = 1.5, alpha = .8)+
  stat_summary(aes(group=NULL, col=NULL), fun.data = mean_se, geom = "point", size = 1.5)+
  scale_x_continuous(breaks = c(0,1), labels = c("Incongruent", "Congruent"), name="",
                     expand = c(.2,.2), minor_breaks = NULL)+
  scale_y_continuous(breaks = seq(0,1,.05), minor_breaks = NULL)+
  coord_cartesian(ylim = c(.60, 1.01))+
                      # Adjust
  theme_minimal()+
  theme(axis.text.x = element_text(size = 10), legend.position = "none")+
