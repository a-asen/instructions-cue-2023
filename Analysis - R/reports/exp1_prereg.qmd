...
`r round(loss$rt_sd_prop + loss$inducer_fail_prop, 2)` percent of the data.
...
d2 |>
  summarise(
    ...
    d                = fmt_APA_numbers( cohensD(pe_FALSE, pe_TRUE, method = "corrected") ),
  ) -> d_pe
...
p1 <- 
  ...
  labs(title = "Response Time (RT)", x = "", y = "Response Time (ms)")
...
p2 <-
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(size = 10), legend.position = "none")+
  labs(title = "Proportion of Error", x = "", y = "Error") 
