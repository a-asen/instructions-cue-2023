...
raw_d <- 
  data |> 
  dplyr::select(id, t %>% rial_info, diagnostic_run, italic, inducer_run, stimulus, congruent, correct_diag_response_side, response, correct_response,rt) |>
...
```
*Note.* The tests have `r  d2_df` degrees of freedom.  <br/>
...
p2 <- 
  ...
  ungroup() |>  
  mutate(con2 = ifelse(con==T,1,0)) |>
  ggplot(aes(x = con2, y = pct, fill = factor(con2)))+
  stat_summary(fun=mean, geom="col")+
  stat_summary(aes(group=NULL), fun.data = mean_se, geom = "errorbar", width=.4, size = 1)+
  scale_x_continuous(breaks = c(0,1), labels = c("Incongruent", "Congruent"), name="",
                     expand = c(.2,.2), minor_breaks = NULL)+
  scale_y_continuous(breaks = seq(0,1,.02), minor_breaks = NULL)+
  coord_cartesian(ylim = c(.9, 1.01))+
                        # Adjust? 
  theme_minimal()+
  theme(axis.text.x = element_text(size = 10), legend.position = "none")+
  labs(title = "Proportion of correct trials", x = "", y = "Proportion")+
  annotate("segment", x=-0.1, xend=1.1, y=.995, yend=.995, size=1)+
  annotate(geom = "text", x = .5, y=.997, label="**", size=5)
    
p1+p2
ggsave("../figs/exp1_rt-pct.jpeg", p1+p2, width=8, height=5)
```
