...
d |> 
  filter(!(is.na(con))) |>
  summarise(.by=c(id,con), 
               rt = mean(rt), 
               pe = 1 - mean(correct_response)) |>
  pivot_wider(names_from = con, values_from=c(rt,pe)) -> dat

dat |> 
  t.test(rt~con, dat = _, paired=T)


fn_test <-\(data, indices){
  d <- data[indices,]
  t.test(d[["rt_FALSE"]], d[["rt_TRUE"]], paired=T) |> tidy()
}

r <- boot::boot(dat, fn_test, 50)
...
#### Paired sample t-test with removed outliers
...
d2 |> 
  ...
  labs(x = "Difference between means", y ="Density")+
  theme(legend.position = "none")
```
