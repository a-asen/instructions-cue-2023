...
d2 |> 
  ...
  geom_density(col="red", linewidth=1.2) 
```
...
#| echo: false
wilcox.test(d2$rt_FALSE, d2$rt_TRUE, paired=T) |>
  tidy() |>
  bind_rows(
    wilcox.test(d2$pe_FALSE, d2$pe_TRUE, paired=T) |>
      tidy()
  )
...
d2 |>
  ...
  filter(rt > (rt_l - 1.5*rt_q), rt < (rt_h + 1.5*rt_q),
         pe > (pe_l - 1.5*pe_q), pe < (pe_h + 1.5*pe_q)) -> d3
...
# Even after excluding outliers, we still find a significant congruency effect
```
