...
gen_data |>
  ...
  mutate(eff = factor(as.character(eff))) |>
  pivot_longer(c(rt.p, pct.p)) |>
  ggplot(aes(size, pct.d, col = eff, group=eff ))+
  stat_summary(geom="line")+
  geom_smooth(method="lm", alpha =.2) -> p2
...
```{r}
gen_data |> 
  # filter(eff<.16, eff>.14) |>
  # filter(size %in% c(10,20,30,40,50,60)) |> 
  filter(size %in% c(20,40,60)) |>
  mutate(size = factor(size)) |>
  ggplot(aes(dia.len, rt.p, col=size, group=size))+
  stat_summary(geom="line")+
  geom_hline(yintercept=.05)
```
