...
load("../data/simulations/diagnostic_length_simulation.rdata")
...
if(!exists("diag_simu") & run_create_simulations){
  ...
  # save(diag_simu, file = "../data/simulations/diag_simu_raw.rdata")
...
if(run_create_animations){
  ...
  # Save
  anim_save("../figs/animations/anim1.gif", p_ani, path="./")
}
```
...
if(run_create_animations){
  walk(seq(min(diag_simu_sum$size), max(diag_simu_sum$size), 1), \(x){
    ...
    p <- 
      ...
      ggsave(plot = p, filename = paste0("../figs/animations/anim2/anim_diag_", x, ".png"),
            width = 7, heigh = 5) 
  })
  
  # get path to graphs
  list.files("../figs/animations/anim2/", full.names = T) -> figfnames
  # fix arrangement
  tibble(figfnames) |>
  ...
  # create gif
  gifski(o_figfnames, "../figs/animations/anim_sample_size_increase.gif", delay = .25)
  
  # create video format
  av_encode_video(o_figfnames, "../figs/animations/anim_sample_size_increase.mkv", framerate = 4)
}
```
...
if( run_create_animations ){
  ...
  gifski(o_figfnames, "../figs/anim/anim_diag_len_increase.gif", delay = .25)
    ...
    ggsave(plot = p, filename = paste0("../figs/animations/anim3/anim_diag_", x, ".png"),
          width = 7, heigh = 5, dpi = 300) 
})
# get list
list.files("../figs/animations/anim3/", full.names = T) -> figfnames
tibble(figfnames) |>
  mutate(arr = as.integer( str_split(figfnames, "[_.]") |> map_chr(5) )) |>
  arrange(arr) |>
  pull(figfnames) -> o_figfnames

gifski(o_figfnames, "../figs/animations/anim_diag_len_increase.gif", delay = .25)

av_encode_video(o_figfnames, "../figs/animations/anim_diag_len_increase.mkv", framerate = 4)
""
```
...
With 34 participants and 70 post-cue trials, we should have a solid 90% power to detect a small effect of .15.
