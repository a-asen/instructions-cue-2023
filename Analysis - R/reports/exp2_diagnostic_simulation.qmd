...
load("../data/_local/diag_simu_raw.rdata")
...
if(!exists("diag_simu") & run_create_simulations){
  ...
  # save(diag_simu, file = "../data/_local/diag_simu_raw.rdata")
  ...
  #             )
  # save(diag_simu_sum, file="../data/simulations/diag_simu_sum.rdata")
...
if(run_create_animations){
  ...
  # Save
  anim_save("../outputs/animations/anim1.gif", p_ani, path="./")
}
```
...
if(run_create_animations){
  walk(seq(min(diag_simu_sum$size), max(diag_simu_sum$size), 1), \(x){
    ...
    p <- 
      ...
      ggsave(plot = p, filename = paste0("../outputs/animations/anim2/anim_diag_", x, ".png"),
            width = 7, heigh = 5) 
  })
  
  # get path to graphs
  list.files("../outputs/animations/anim2/", full.names = T) -> figfnames
  # fix arrangement
  tibble(figfnames) |>
  ...
  # create gif
  gifski(o_figfnames, "../outputs/animations/anim_sample_size_increase.gif", delay = .25)
  
  # create video format
  av_encode_video(o_figfnames, "../outputs/animations/anim_sample_size_increase.mkv", framerate = 4)
}
```
...
if( run_create_animations ){
  walk(seq(56, max(diag_simu_sum$dia.len), 2), \(x){
    ...
    diag_simu_sum |>
      ...
      ggsave(plot = p, filename = paste0("../outputs/animations/anim3/anim_diag_", x, ".png"),
            width = 7, heigh = 5, dpi = 300) 
  })
  
  # get saved plots
  list.files("../outputs/animations/anim3/", full.names = T) -> figfnames
  tibble(figfnames) |>
    mutate(arr = as.integer( str_split(figfnames, "[_.]") |> map_chr(5) )) |>
    arrange(arr) |>
    pull(figfnames) -> o_figfnames
  ...
  # Create GIF
  gifski(o_figfnames, "../outputs/animations/anim_diag_len_increase.gif", delay = .25)
  
  # Create video file
  av_encode_video(o_figfnames, "../outputs/animations/anim_diag_len_increase.mkv", framerate = 4)
}
```
...
We have two solutions to improve power: (1) Increase the sample size or (2) increase the number of diagnostic trials. Regarding the diagnostic trials, we are weary of adding too many diagnostic trials given that the cue is supposed to signal "prepare" for the switch. Increasing the number of diagnostic trials might influence the usefulness of the cue. 

The plot below visualizes the effect size across various diagnostic length and effect sizes for a sample size of 30.  
...
diag_simu_sum |> 
  ...
  scale_y_continuous(breaks = seq(0, 100, .1) ) +
  annotate("text", x = diag_length+20, y = .1,
           label=paste("Dashed:", diag_length*.75))+
  labs(
    x = "Diagnostic length", 
    y = "Power",
...
Increase diagnostic length to 70 (the point between 60 and `r 60/.75` - the point to exactly counteract the loss of trials, 60/.75).
...
diag_simu_sum |> 
  filter(dia.len %in% diag_length & size %in% samp_size) |>
  filter(pct.pow > power_90[1] & pct.pow < power_90[2]) |>
  filter(eff %in% effect_sizes) |>
  pull(pct.pow) -> desc_power
...
With `r if(length(samp_size>1), paste(samp_size, collapse = " & "), samp_size)` participants and `r diag_length` post-cue trials, we  will have a power of `r if(length(desc_power>1), paste(paste(round(desc_power,2), collapse=" & "), "respectively"), round(desc_power,2))` with an effect size of `r effect_sizes`.

It should be noted that it might not be necessary to expect such a small effect size. Indeed, the integrated scores suggest that we may expect an effect size of about .4. Which would show a significant effect for even 20 participants and 45 trials:
...
Expecting an effect size of .4 would essentially mean that we could detect a significant effect with very few participants and very few observations. For our purposes, however, we will attempt to find an effect for response times which was small in the first experiment. 
