...
rbind(d_rt, d_pct) |>
  ...
  cols_label(   b.est = md("*M*"), bf = html("BF<sub>10</sub>"), hdi = "HDI",
                Mdiff = html("<i>M</i><sub>diff</sub>") )  |>
  cols_move(    "ps", Mdiff ) |> 
...
d |>
  ...
  stat_summary(position=position_dodge(.1), fun.data = mean_se, alpha = .3)+
  stat_summary(position=position_dodge(.1), geom = "line", fun.data = mean_se, alpha = .3)+
  stat_summary(aes(group=NULL), position=position_dodge(.05), fun.data = mean_se, col="red")+
  stat_summary(aes(group=NULL), position=position_dodge(.05), geom = "line", fun.data = mean_se, col="red")+
  scale_y_continuous(breaks = seq(0,2000,25), expand = c(.1,.1), minor_breaks = NULL)+
  scale_x_continuous(breaks = c(0,1), labels = c("Incongruent", "Congruent"), name="",
                     expand = c(.2,.2), minor_breaks = NULL)+
  theme_minimal()+
  theme(axis.text.x = element_text(size = 14))+
  labs(title = "Response time (ms)", x = "", y = "Response time (ms)") ->
  p1
...
d |>
  ...
  ungroup() |>   ungroup() |>
  mutate(con = ifelse(congruent==TRUE,1,0)) |>
  ggplot(aes(x = con, y = pct, group = id))+
  stat_summary(position=position_dodge(.1), fun.data = mean_se, alpha = .3)+
  stat_summary(position=position_dodge(.1), geom = "line", fun.data = mean_se, alpha = .3)+
  stat_summary(aes(group=NULL), position=position_dodge(.05), fun.data = mean_se, col="red")+
  stat_summary(aes(group=NULL), position=position_dodge(.05), geom = "line", fun.data = mean_se, col="red")+
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(size = 14))+
  labs(title = "Proportion correct trials", x = "", y = "Proportion") -> 
  p2
