...
d2 |> 
  mutate( diff = pct_FALSE-pct_TRUE)  
...
d2 |>
  summarise(
    ...
    conf = t.test(  pct_FALSE, pct_TRUE, paired = TRUE, alternative = "less" )[["conf.int"]] |> pull(),
...
t.test(d2$rt_FALSE, d2$rt_TRUE)$conf.int |> as.character() |> str_remove(pattern = " ")#str_trim()#gsub(pattern = "", replacement="")
...
attributes(t.test(d2$rt_FALSE, d2$rt_TRUE)$conf.int)
sloop::ftype()
test$conf.int
test$stderr
test$estimate
...
d2 |> 
  pivot_wider(names_from = congruent, values_from = c(rt, pct)) ->
  d3 
BayesFactor::ttestBF(d3$rt_FALSE, d3$rt_TRUE, paired = T, iterations = 10000, ) -> test
...
if(d2_df == floor(d2_df)) "NOT SIMILAR, CHECK DF"
  # degrees of freedom
