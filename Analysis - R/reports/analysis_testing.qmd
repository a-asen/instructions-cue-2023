...
lisas_ind |>
  left_join(lisas_grand, by="id") |>
  mutate(lisas_IN   = rt_FALSE + (g_rt_sd/g_pe_sd) * pe_FALSE,
         lisas_CON  = rt_TRUE + (g_rt_sd/g_pe_sd) * pe_TRUE,
         ) ->
  lisas
```
...
lisas |>
  rowwise() |>
  mutate(l_diff = lisas_IN - lisas_CON) |> 
  ungroup() |> 
  summarize(
    m_incongruent   = fmt_APA_numbers( mean(lisas_IN) ),
    m_congruent     = fmt_APA_numbers( mean(lisas_CON) ),
    p               = t_lisas$p.value,
    ...
    d               = fmt_APA_numbers( cohensD(lisas_IN, lisas_CON, method="corrected") )
    ) -> lisas_t
...
lisas |> 
  pivot_longer(c(lisas_IN, lisas_CON)) |> 
  mutate(name = ifelse(name=="lisas_IN", "Incongruent", "Congruent")) |>
...
## Inducer encoding/RT (& correlation)
```{r sum data}
#| echo: false
...
# Correlation for each person: 
d_ind |>
  group_by(id) |>
  summarize(cor = cor(trial, instructions),
            encoding =  mean(instructions),
            encoding_sd = sd (instructions),
            encoding_skew = moments::skewness(instructions), 
            trial = mean(trial),
            trial_sd = sd(trial, na.rm = T)) 

d_ind |>
  group_by(id) |>
  mutate(instructions2 = log(instructions)) |>
  ggplot(aes(id, instructions2))+
  geom_point()
  summarize(
            encoding =  mean(instructions2),
            encoding_sd = sd (instructions2),
            encoding_skew = moments::skewness(instructions2), 
  ) 
...
Response time in milliseconds (ms) for encoding the instructions and inducer trial.
```{r within plot}
#| fig-dpi: 300
#| echo: false
#| error: false
...
d_ind |>
  ggplot(aes(x=instructions, y=trial, col=id))+
  geom_line(stat="smooth", method=lm, alpha = .7)+
  # grand line
  #geom_smooth(aes(fill=NULL), method=lm, col="red")+
  #geom_line(aes(fill=NULL), stat="smooth", method=lm, alpha = .8, col="red", linewidth=1)
  scale_y_continuous(breaks=seq(0,2000,250))+
  scale_x_continuous(breaks=seq(0,20000,1000))+
  coord_cartesian(ylim=c(0,2000))+
  theme_minimal()+
  theme(legend.position="none")+
  labs(x="Encoding", y="Trial")
...
# difference between correct and incorrect inducers ? 

# exploring
d_ind |> 
  group_by(id, `correct_response_Inducer trial`) |>
  summarize(cor = cor(instructions, trial),
...
test <- 
  ind_within_split |>
  pivot_wider(names_from=`correct_response_Inducer trial`, values_from=c(instructions,trial, cor))
...
# but  why would we test this? 
trial     <- t.test(test$trial_0, test$trial_1, paired=T)
encoding  <- t.test(test$instructions_0, test$instructions_1, paired=T)
cor       <- t.test(test$cor_0, test$cor_1, paired=T)

ind_within_split |>
  ggplot(aes(x = factor(`correct_response_Inducer trial`), y=cor))+
  geom_point(position=position_jitter(width = .1), alpha =.35)+
  stat_summary(col="red")+
  labs(title="Correlation over correct inducer response", x="Correct inducer", y="Correlation")
...
ind_within_split |>
  ggplot(aes(x = factor(`correct_response_Inducer trial`), y=trial))+
  geom_point(position=position_jitter(width = .1), alpha = .35)+
  stat_summary(col="red")+
  labs(title="Response time over correct inducer response", x="Correct inducer", y="Response time")
```
...
ind_within_split |>
  ggplot(aes(x = factor(`correct_response_Inducer trial`), y=instructions))+
  geom_point(position=position_jitter(width = .1), alpha = .35)+
  stat_summary(col="red")+
  labs(title="Encoding time over correct inducer response", x="Correct inducer", y="Encoding time (ms)")
