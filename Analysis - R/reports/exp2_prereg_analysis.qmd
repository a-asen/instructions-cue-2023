...
trns_mod(pe_aov) |> 
  ...
  cols_label(
    `F (1,34)_BIS` = md("*F*(1,34)"), `F (1,34)_LISAS` = md("*F*(1,34)"), `F (1,34)_Response time` = md("*F*(1,34)"),
    `F (1,34)_Error rate` = md("*F*(1,34)"), p_LISAS = md("*p*"), p_BIS = md("*p*"), `p_Error rate`= md("*p*"), 
    `p_Response time` = md("*p*"), `pes_Error rate` = "n^2", `pes_Response time` = "n^2", pes_LISAS = "n^2", 
    `pes_BIS`="n^2", names = "Variables") |>
...
rep$pe_34 |> 
  ...
  cols_label(
    minBF_Error = md("minBF~10~"), maxBF_Error = md("maxBF~10~"), minBF_LISAS = md("minBF~10~"), 
    maxBF_LISAS = md("maxBF~10~"), minBF_BIS = md("minBF~10~"), maxBF_BIS = md("maxBF~10~"), 
...
pbapply::pblapply(1:35, \(x){
  b <- brm(pe ~ congruent + post_cue + congruent:post_cue + (1|id), 
           d_ex_test, save_pars = save_pars(all=T),
           iter = 6000, chains = 6, backend = "cmdstanr", cores = 6, init=0)

  fixef(b) |> 
    as_tibble() |> 
    mutate(.before=1, names = rownames(fixef(b)), 
           across(where(is.double), ~fmt_APA_numbers(.x, .low_val=T)), 
           Est.Error=NULL)
})
