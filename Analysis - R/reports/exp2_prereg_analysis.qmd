...
```{r half split test, echo=FALSE}
time_intr_fq <- lmer(rt ~ post_cue*congruent*inducer_run + (1|id), d_ex |> filter(trial_info=="Diagnostic trial"))
summary(time_intr_fq)
```

```{r}
time_intr_ba <- brm(
  rt ~ congruent*post_cue*inducer_run, d_ex |> filter(trial_info=="Diagnostic trial"),
  init=0, chains = 6, iter= 6000, backend = "cmdstanr", cores = 6)

bayes_plot(time_intr_ba, c("b_congruentTRUE", "b_post_cueTRUE", "sigma", "b_congruentTRUE:post_cueTRUE"), par_match = F)

map(colnames(as.matrix(time_intr_ba))[2:8], \(x){
  paste(x, 
  coef_hdi(as.matrix(time_intr_ba)[,x])
  )
}) 

coef_hdi(as.matrix(time_intr_ba)[,"b_congruentTRUE:post_cueTRUE:inducer_run"])
```
