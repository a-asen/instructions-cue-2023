...
coefs$rt <- bayes_coefs_to_table(co_bay$rt, "rt", c("r_id", "z_1", "sd_id"))
```
...
coefs$pe <- bayes_coefs_to_table(co_bay$pe, "pe", c("r_id", "z_1", "sd_id"))
```
...
coefs$lisas <- bayes_coefs_to_table(co_bay$lisas, "lisas", c("r_id", "z_1", "sd_id"))
```
...
coefs$bis <- bayes_coefs_to_table(co_bay$bis, "bis", c("r_id", "z_1", "sd_id"))
```
...
if(run_bayesian_models){
  b_rm_out <- list() 
  b_rm_out$rt <- bay_out_rm( d_ex_test |> filter(!(id %in% id_is_outlier)), "rt" )
  b_rm_out$pe <- bay_out_rm( d_ex_test |> filter(!(id %in% id_is_outlier)), "pe" )
  b_rm_out$lisas <- bay_out_rm( d_ex_test |> filter(!(id %in% id_is_outlier)), "lisas" )
  b_rm_out$bis <- bay_out_rm( d_ex_test |> filter(!(id %in% id_is_outlier)), "bis" )
...
b_rm_out |> 
  ...
  mutate(
    ...
    across(c("Estimate", contains("Q")), ~fmt_APA_numbers(.x, .chr=T))
  ) |>
  gt(groupname_col = "group") |>
...
```{r post-cue RT and PE split by inducer run, echo=FALSE}  
d_ex |> 
  filter(post_cue==T) |>
  mutate(
    .by = c(id, inducer_run),
...
```{r add bis-lisas to inducer-calced, echo=FALSE}
d_ex_exp_test <- d_ex |>
  filter(trial_info=="Diagnostic trial") |>
  summarise(
...
d_ex_exp_test |>
  ...
  ggplot(aes(post_cue, bis, col=congruent))+
  facet_wrap(~three) +
  stat_summary()
```


```{r bis quater-split, echo=FALSE}
d_ex_exp_test |>
  mutate(
     three = case_when(
      inducer_run > 2*(max(inducer_run)/3)   ~ "Third", 
...
brm_3_way_test <- function(data, dep){
  time_intr_ba <- brm(
    as.formula(glue::glue("{dep} ~ congruent*post_cue*inducer_run + (1|id)")),
    data,
...
```{r interaction 3 way table, echo=FALSE}
...
```{r 3 way interaction table done, echo=FALSE}
intr3_to_table(fq_intr3way$rt, "rt") |>
  bind_rows(
    intr3_to_table(fq_intr3way$pe, "pe"),
  ) |> 
  bind_rows(
    intr3_to_table(fq_intr3way$lisas, "lisas"),
  ) |>
  bind_rows(
    intr3_to_table(fq_intr3way$bis, "bis"),
  ) |>
  filter(!(vars=="(Intercept)")) |>
  left_join(
    bayes_coefs_to_table(ba_intr3way$rt, "rt", c("r_id", "sd_id")) |>
      bind_rows(
        bayes_coefs_to_table(ba_intr3way$pe, "pe", c("r_id", "sd_id"))
      ) |>
      bind_rows(
        bayes_coefs_to_table(ba_intr3way$lisas, "lisas", c("r_id", "sd_id"))
      ) |>
      bind_rows(
        bayes_coefs_to_table(ba_intr3way$bis, "bis", c("r_id", "sd_id")) 
      ),
    by = join_by("vars"=="names", "group")
  ) |>
  mutate(
    group = case_when(
      group=="rt"~"Response time",
    ...
    vars = case_when( 
      vars=="post_cueTRUE" ~ "Cue", 
      vars=="congruentTRUE" ~ "Congruency", 
      vars=="inducer_run" ~ "Time", 
      vars=="congruentTRUE:post_cueTRUE" ~ "Congruency x Cue", 
      vars=="congruentTRUE:inducer_run" ~ "Congruency x Time", 
      vars=="post_cueTRUE:inducer_run" ~ "Cue x Time", 
      vars=="congruentTRUE:post_cueTRUE:inducer_run" ~ "Cue x Congruency x Time", 
    ),
    `Std. Error` = NULL,
  ...
  mutate(.before=1, g=group, group=NULL) |>
  gt() |>
  tab_spanner("HDI", c(Q2.5, Q97.5)) |>
  tab_spanner("", c(Estimate.x, df, "t value", "Pr(>|t|)")) |>
  tab_spanner("Frequentist", c(Estimate.x, df, "t value", "Pr(>|t|)")) |>
  tab_spanner("Bayesian", c(Estimate.y, pdir, erat, Q2.5, Q97.5)) |>
  cols_align("center", 3:12) |>
  cols_move(zz, "Pr(>|t|)") |>
  cols_label(zz="",
    Estimate.x = md("*M*~diff~"),"t value" = md("*t*"), "Pr(>|t|)" = md("*p*"),
    Estimate.y = md("*b*"), pdir=md("*p*~b~"), erat = md("ER~b~"),
    Q2.5 = "Low", Q97.5 = "High", vars = "Predictors"
  ) -> int_tab
