...
trns_mod <- function(data){
  ...
  d |> 
    ...
    mutate(`F (1,34)` = fmt_APA_numbers(`F (1,34)`, .chr=T),
           p = fmt_APA_numbers(p, .p=T),
           pes = fmt_APA_numbers(pes, .low_val=T))
}

tibble(names = "Error", `F (1,34)`=NA, p=NA, pes=NA) |>
  bind_rows(
    trns_mod(pe_aov) ) |>
  bind_rows(
...
# Bayesian rmANOAV
bay_res <- anovaBF(pe ~ post_cue * congruent + id, 
                   whichRandom = "id", 
...
hdi(test)
# Doesnt give posterior
extractBF(bay_res)
bayestestR::hdi(bay_res)
...
bayes_plot(bay_res2, c("z_", "sigma"))

bayestestR::bayesfactor(bay_res2)
bayestestR::hdi(bay_res2)

bayesplot::mcmc_intervals(as.matrix(bay_res2))
...
```{r}
data_outliers$id[data_outliers$is.extreme==T]
data_outliers$id[data_outliers$is.outlier==T] |> unique()
```

### Outliers

```{r raw outliers, echo=FALSE}
d_ex_test |> 
  mutate(Cue = fct_relevel(ifelse(post_cue==T, "Post-cue", "Pre-cue"),"Post-cue", after=1),
         Congruency = fct_relevel(ifelse(
           congruent==T, "Congruent", "Incongruent"), "Congruent", after=1)) |>
...
is_outlier <- outlier_values |> filter(is.outlier==T) |> pull(id) |> unique()
is_extreme <- outlier_values |> filter(is.extreme==T) |> pull(id) |> unique()
```

```{r test data without outliers, echo=FALSE}

aov_ez_rm_out <- ez::ezANOVA(
  d_ex_test |> filter(!(id %in% is_outlier)), 
  pe, id, within = c(post_cue, congruent), detailed = T)

aov_ez_rm_out$ANOVA |> 
  mutate(across(c(3:6,9), ~fmt_APA_numbers(.x, .chr = T)),
         p = fmt_APA_numbers(p, .p=T)) |> 
  gt()
```
