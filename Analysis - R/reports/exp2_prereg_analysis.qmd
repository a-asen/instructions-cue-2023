...
par_test("rt") |>
  ...
  cols_label(starts_with("SE") ~ ("SE"), starts_with("estimate") ~ "Estimate",
...
bayes_plot(co_bay$lisas, c("z_", "sigma"))    # coefficient plot
...
bay_rep_test <- function(dep){
  pblapply(1:2, \(x){
    ...
    coefs <- 
      ...
      mutate(
        .after="Estimate", 
        pdir = bay_eff_prob(b, variables(b)[1:4]),
        erat = bay_erate(b, variables(b)[1:4]),
      ) |>
      filter(!(names=="Intercept"))
