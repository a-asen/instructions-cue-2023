...
```{r}
exp2_sim2x2x2 <- function(N, blocks, block.len, e.con, e.cue, e.cue.long, short.post.con, long.post.con){
  #' @param N              Number of subjects
  #' @param blocks         Number of blocks 
  #' @param block.len      Length of each block
  #' @param e.con          What is the effect on RT before the *cue* during the *short CSI*?
  #' @param e.cue          What is the effect on RT after the *cue* during the *short CSI*?
  #' @param e.cue.long     What is the effect on RT after the *cue* during the *long CSI*?
  ...
  tibble(
    ...
    con    = rep.int( c("con", "incon"), N * blocks * block.len),
    rt     = rnorm(   N * blocks * block.len * cue_rep, 
                      0 +
                        # Main effect of con:
                          # i.e., what is the effect on  pre-cue during the short CSI?
                        - as.integer(con=="con") * e.con +
                        # Main effect of cue:  
                          # i.e., what happens after the cue appears?
                        + as.integer(cue=="post") * eff.cue +
                        
                        # Interaction between CSI & CUE: 
                          # I.e., what happens after the cue with long CSI?
                        as.integer(cue=="post" & csi =="long") * e.cue.long + 
                        
                        # Interaction between con-post over CSI: 
                        # Primary interest of the investigation:
                          # I.e., what happens to the congruency after the cue for the different CSI?
                        - as.integer(con=="con" & csi=="short" & cue=="post") * short.post.con + # Reduce some RT more
                        - as.integer(con=="con" & csi=="long"  & cue=="post") * long.post.con
    )) -> d
...
```{r}
pblapply(1:30, \(i) exp2_sim2x2x2(30, 24, 20, .02, .05, 0.07, .13, .17) |> mutate(v=i) ) |> 
  map_df(~.x) -> par30_csi

par30_csi |> 
  group_by(id, csi, cue, con) |>
  summarise(rt=mean(rt)) -> par30_csi_sum
...
exp2_sim2x2x2(30, 24, 20, .02, .05, 0.07, .13, .17) |>
  aov_car(rt ~ csi*cue*con + Error(id/(csi*cue*con)), data = _,) -> test
test
...
pblapply(10:100, \(x){
  ...
  purrr::map_df(1:500, \(y){
    # repeat each par-size 100 times
    exp2_sim2x2x2(30, 24, 20, .02, .05, 0.07, .13, .17) -> d
    d |> 
      aov_car(rt ~ csi*cue*con + Error(id/(csi*cue*con)), data = _,) -> res
