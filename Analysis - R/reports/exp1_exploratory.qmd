...
colnames(data)
data |> 
  select(id, trial_info, diagnostic_run, inducer_run, stimulus, congruent, correct_diag_response_side, response, correct_response,rt) |>
  filter(trial_info=="Diagnostic trial") |>
  group_by(id, correct_diag_response_side, congruent) |> 
  summarize(rt = mean(rt, na.rm=T), 
            acc = mean(correct_response, na.rm=T)) |>
  ggplot(aes(x=correct_diag_response_side, rt))+
