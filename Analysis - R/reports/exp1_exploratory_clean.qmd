...
mod1 <- lmer(rt ~ con + (1|id), 
             an_d |> filter(trial_info=="Diagnostic trial") )
mod2 <- lmer(rt ~ con * trial + (1|id),
             an_d |> filter(trial_info=="Diagnostic trial") )
mod3 <- lmer(rt ~ con * instructions + (1|id),
             an_d |> filter(trial_info=="Diagnostic trial") )
# anova(mod1, mod2)
# anova(mod1, mod3)
# anova(mod2, mod3)
...
# summary(mod1)
summary(mod2)
  # rt is adjusted by inducer RT by 0.05 * inducer_rt
summary(mod3)
  # rt is adjusted by instruction time by 0.005 * instructions

#' No interaction between congruency and instruction/inducer rt. Suggesting that
#' more preparation/inducer rt generally slows RT on the diagnostic task.
```

No interaction between congruency and instruction/inducer rt. Suggesting that more preparation/inducer rt generally slows RT on the diagnostic task.
