...
Positive correlation indicate that more time spent on reading the instructions increased response times on the diagnostic trials. 
...
p2 <- 
  ...
  filter( !is.na(`rt_Inducer trial`) ) |>
  filter( !is.na(`rt_Inducer instructions`) ) |>
  ggplot(aes(instructions, trial, col=)) +
  geom_point(alpha=.1) +
  geom_smooth(method=lm, col="indianred2", linewidth=1) +
  scale_x_continuous(breaks=seq(0,20000,2000)) +
  scale_y_continuous(breaks=seq(0,2000,250)) +
...
Note that this is a comparison of ANOVAs, and not of linear mixed-models. The latter may be prefered over the former. 


### Linear mixed-model:

We can do a linear mixed model to investigate whether encoding interacted with congruency. 
I.e., more time encoding the instructions lead to stronger congruency effect. 

Here we log transform the response time variable due to non-normality. 
```{r}
#| echo: false
#| message: false
reg_d <- 
  ...
  summarise(rt = mean(rt), instructions = unique(instructions), pct = mean(correct_response)) |>
...
```
Results suggest that only the model that include encoding, but no the interaction, is a significant improvement compared to the former model. 

This suggest that encoding time did not influence the congruency effect. 


#### Random block effect
Is it necessary to include a random effect of block (inducer run) per subject? 

In general, we do not expect block to have a unique effect on RT, although a general decrease with increasing practice. 
...
There does indeed appear to be a general decrease in RT following block. Including a random intercept would probably fit the data better. 
...
The model does account for more variance, but does not change the general pattern. 

Generally, we believe that the effect of block stem from time-on-task / practice, and does not really change the outcome.  


Regardless, non of the selected models include the interaction between congruency and encoding. 
This suggest that congruency does not interact with encoding. 
This should not be interpreted as a major finding. Our main investigation did not encompass the interaction between encoding and congruency, but is meant as exploratory. 


### Trend in congruency across block
...
In this way, including a random intercept for each block explains more variance. 



## Impact of Encoding on  PCT
...
reg_d |> 
  betareg::betareg(pct ~ con | id, data = _)
p_mod1 <- 
  reg_d |>
  glmer(pct ~ con + (1 | id), data = _,
