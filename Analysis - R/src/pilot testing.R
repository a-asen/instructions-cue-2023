...
#   Behaviour       =====
data |>
  select(id, trial_info, inducer_run, correct_response, rt, congruent) |> view()
...
## RT
aov(rt ~ congruent, d2) |> summary()

## PCT
aov(pct ~ congruent, d2) |> summary()

## Plot
d2 |>
  ggplot(aes(x = congruent, y = rt))+
  geom_point(position = position_nudge(x=-.1), alpha = .5)+
